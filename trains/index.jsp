...
<head>
  ...
  <title>LankaRails | Train Management</title>
  ...
  <div class="flex-1 flex flex-col md:ml-60">
    ...
    <main class="flex-1 p-6 space-y-6">
      ...
      <div class="flex flex-col md:flex-row md:justify-between md:items-center space-y-4 md:space-y-0">
        <a href="${pageContext.request.contextPath}/staff/train?action=create"
           class="flex items-center gap-2 px-5 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg font-semibold transition">
          <i class="fas fa-train"></i> Add New Train
        </a>

        <div class="relative w-full md:w-1/3">
          <input id="search" type="text" placeholder="Search trains..."
        ...
        <table class="w-full text-sm text-left">
          <thead class="text-xs uppercase bg-slate-800 text-slate-300">
            <tr>
              ...
              <th class="px-6 py-4">Train Number</th>
          ...
          <tbody id="table" class="text-slate-300">
            <c:choose>
              <c:when test="${not empty trains}">
                <c:forEach var="train" items="${trains}">
                  <tr class="border-b border-slate-800 hover:bg-slate-800/50">
                    <td class="px-6 py-4">${train.trainId}</td>
                    <td class="px-6 py-4">${train.trainNumber}</td>
                    <td class="px-6 py-4">${train.totalSeats}</td>
                    <td class="px-6 py-4 text-center space-x-3">
                      <a href="${pageContext.request.contextPath}/staff/train?action=edit&id=${train.trainId}" class="text-green-400 hover:text-green-500">
                        <i class="fas fa-edit"></i>
                      </a>
                      <a href="${pageContext.request.contextPath}/staff/train?action=delete&id=${train.trainId}" onclick="return confirmDelete(this.href);" class="text-red-400 hover:text-red-500">
              ...
              <c:otherwise>
                <tr>
                  <td colspan="4" class="px-6 py-4 text-center text-gray-400">No trains found.</td>
                </tr>
              </c:otherwise>
            </c:choose>
          </tbody>
