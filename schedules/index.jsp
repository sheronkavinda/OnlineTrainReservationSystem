...
<head>
  ...
  <title>LankaRails | Schedule Management</title>
  ...
  <div class="flex-1 flex flex-col md:ml-60">
    ...
    <main class="flex-1 p-6 space-y-6">
      ...
      <div class="flex flex-col md:flex-row md:justify-between md:items-center space-y-4 md:space-y-0">
        <a href="${pageContext.request.contextPath}/staff/schedule?action=create"
              ...
              <c:when test="${not empty schedules}">
                <c:forEach var="s" items="${schedules}">
                  <tr class="border-b border-slate-800 hover:bg-slate-800/50">
                    <td class="px-6 py-4">${s.scheduleId}</td>
                    <td class="px-6 py-4">${s.trainNumber}</td>
                    <td class="px-6 py-4">${s.startStation} → ${s.endStation}</td>
                    <td class="px-6 py-4">${s.departureTime}</td>
                    <td class="px-6 py-4">${s.arrivalTime}</td>
                    <td class="px-6 py-4">Rs. ${s.ticketPrice}</td>
                    <td class="px-6 py-4 text-center space-x-3">
                      <a href="${pageContext.request.contextPath}/staff/schedule?action=edit&id=${s.scheduleId}" class="text-green-400 hover:text-green-500">
                        <i class="fas fa-edit"></i>
                      </a>
                      <a href="${pageContext.request.contextPath}/staff/schedule?action=delete&id=${s.scheduleId}" onclick="return confirmDelete(this.href);" class="text-red-400 hover:text-red-500">
